...
if ((Find-Module -Name SqlServer).Count -eq 0) { 
	Install-Module SqlServer 
}
if ((Find-Module -Name PowerShellGet).Count -eq 0) { 
...
function Publish-Required {                  
    if (!(Test-Path $appDataPath)){
        Write-Debug "Database path not found! $appDataPath"
		New-Item $appDataPath -ItemType Directory
    }                
