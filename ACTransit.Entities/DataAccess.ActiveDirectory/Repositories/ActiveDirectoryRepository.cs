...
// ReSharper disable once InconsistentNaming
        ...
        {       
            ...
            //var filterStr = CreateFilter(userFilter, "(memberOf:1.2.840.113556.1.4.1941:=" + group.DistinguishedName + ")");
            //var filterStr = CreateFilter(userFilter, String.Format("(memberOf:{0}:={1})",LDAP_MATCHING_RULE_IN_CHAIN,group.DistinguishedName));
        ...
        public Group[] GetGroups(Group filter)
        {
            ...
            //    using (var ds = new DirectorySearcher(deParent, filterStr, Group.Columns, SearchScope.Subtree))
            //    {
        ...
        //private List<string> GetGroupsOfUserRecursive(string samAccountName)
        //{
        //    var userFilter = new Entity { SamAccountName = samAccountName };
        //    var filterStr = CreateFilter(userFilter);

        //    var result = new List<string>();
        //    using (var deParent = CreateDirectoryEntry())
        //    {
        //        deParent.AuthenticationType = _authenticationType;
        //        using (var ds = new DirectorySearcher(deParent, filterStr, new[] { "memberof" }, SearchScope.Subtree))
        //        {
        //            ds.PageSize = 1000;
        //            using (SearchResultCollection src = ds.FindAll())
        //            {
        //                foreach (SearchResult sr in src)
        //                {
        //                    if (sr.Properties["tokenGroups"] != null)
        //                    {
        ...
        //                            using (var sidSearcher = new DirectorySearcher())
        //                            {

        //                                sidSearcher.SearchRoot = deParent;
        ...
        //                                if (sidResult != null)
        //                                {
        //                                    var str = sidResult.Properties["distinguishedName"][0] as string;
        //                                    if (!string.IsNullOrWhiteSpace(str) && str.Contains("CN="))
        //                                    {
        //                                        var startIndex = str.IndexOf("CN=", StringComparison.Ordinal) + 3;
        //                                        var endIndeex = str.IndexOf(',', startIndex);
        //                                        var len = (endIndeex > startIndex)
        //                                            ? endIndeex - startIndex
        ...
        //private List<string> GetGroupsOfUserRecursiveNew(string samAccountName)
        //{
        ...
        //        return null;
        //    var groupFilter = new Group();
        //    string filterStr = CreateFilter(groupFilter, String.Format("(member:{0}:={1})", LDAP_MATCHING_RULE_IN_CHAIN, user.DistinguishedName));

        //    var result = GetEntities(filterStr, groupFilter.GetActiveDirectoryColumns());
        //    var groups = result.Select(m => m.GetCorrespondingObject() as Group);
        //    return groups.Select(m => m.Name).ToList();
        //}
