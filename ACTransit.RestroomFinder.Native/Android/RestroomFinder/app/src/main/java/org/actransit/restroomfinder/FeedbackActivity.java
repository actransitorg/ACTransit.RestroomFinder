...
public class FeedbackActivity extends BaseActivity {
    ...
    Button btnShowHide;
    Switch chkNeedCleaning;
    Switch chkNeedSupplies;
    Switch chkNeedRepair;
    Switch chkRestroomClosed;
    Switch chkInspectionPass;
    ...
    private void setVariables(){
        ...
        restrooom=r.value;
        chkInspectionPass = (Switch)findViewById(R.id.chkInspectionPass);
        txtFeedback = (EditText) findViewById(R.id.txtFeedback);
        lstFeedbacks = (ListView) findViewById(R.id.lstFeedbacks);
        chkNeedCleaning = (Switch)findViewById(R.id.chkNeedCleaning);
        chkNeedSupplies = (Switch)findViewById(R.id.chkNeedSupplies);
        chkNeedRepair = (Switch)findViewById(R.id.chkNeedRepair);
        chkRestroomClosed = (Switch)findViewById(R.id.chkRestroomClosed);
        ratingbarFeedback = (RatingBar)findViewById(R.id.ratingbarFeedback);
        btnShowHide = (Button)findViewById(R.id.btnShowHide);
        lblTitle = (TextView)findViewById(R.id.lblTitle);
        lblLabelId = (TextView)findViewById(R.id.lblLabelId);

        lblListHeader = new TextView(this);

        lblTitle.setText(restrooom.restroomName);

        lblLabelId.setText(restrooom.labelId);
        lblLabelId.setVisibility((restrooom.labelId==null || restrooom.labelId.isEmpty()) ? View.GONE : View.VISIBLE);
        chkInspectionPass.setVisibility((Common.canAddRestroom || Common.canEditRestroom)?View.VISIBLE:View.INVISIBLE);
    ...
    private void Submit(){
        ...
        feedback.needCleaning = chkNeedCleaning.isChecked();
        feedback.needSupply = chkNeedSupplies.isChecked();
        feedback.needRepair = chkNeedRepair.isChecked();
        feedback.closed = chkRestroomClosed.isChecked();
        feedback.needAttention = feedback.getNeedAttention();
        if (Common.canEditRestroom || Common.canAddRestroom){
            feedback.inspectionPassed = chkInspectionPass.isChecked();
        }

        //feedback.rating = new Double(ratingbarFeedback.getRating());

        feedback.restroomId = restrooom.restroomId;
        feedback.badge = AppStorage.Current(this).getBadge();

        //if (feedback.feedbackText.equals("") && feedback.rating<3){
        if (feedback.feedbackText.equals("") || feedback.feedbackText.length()<4){
            Toast(Constants.Messages.feedbackEmptySubmit,1000);
            return;
        }
